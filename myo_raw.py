...
class BT(object):
    ...
    def recv_packet(self, timeout=None):
        ...
        while timeout is None or time.time() < t0 + timeout:
            ...
            c = self.ser.read()
            if not c: 
                print('self.ser.read() returned none instead of a char')
                return None
    ...
    def recv_packets(self, timeout=.5):
        ...
        while time.time() < t0 + timeout:
            ...
            if not p: 
                if len(res) == 0:
                    print('not receiving packets')
                    continue
                return res

            res.append(p)
        print('recv_packets timed out; problem with bluetooth connection?')
        return res
    ...
    def send_command(self, cls, cmd, payload=b'', wait_resp=True):
        ...
        if cmd == 0: print('writing cmd 0 to self.ser.write(s)')
        self.ser.write(s)
...
class MyoRaw(object):
    ...
    def run(self, timeout=None):
        if self.bt.recv_packets(timeout) == []:
        ...
        def handle_data(p):
            ...
            if attr == 0x27:
                ...
                try:
                    ...
                    print('invalid data for attr 0x23:', ':'.join(x.encode('hex') for x in pay))
            else:
...
if __name__ == '__main__':
    ...
    def axis_handler(quat, acc, gyro):
        str = ''
    ...
    #m.add_imu_handler(axis_handler)

    def acc_handler(quat, acc, gyro):
        data = json.dumps({"quat": quat, "acc": acc, "gyro": gyro})
        data += '\n'
        server.send_data(data)
